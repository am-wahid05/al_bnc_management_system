...
class ReceiptService {
  ...
  Future<Uint8List> buildPdf(Delivery delivery, {String companyName = 'AL-BNC Ventures'}) async {
    ...
    document.addPage(pw.Page(
      ...
      build: (context) => pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
        ...
        pw.Table.fromTextArray(headers: const ['Bag', 'Weight'], data: [for (var index = 0; index < delivery.bagWeights.length; index++) ['${index + 1}', '${delivery.bagWeights[index].toStringAsFixed(1)} kg']]),
  ...
  Future<void> print(Delivery delivery) async {
    final bytes = await buildPdf(delivery);
    await Printing.layoutPdf(onLayout: (_) async => bytes, name: 'ALBNC_Receipt_${delivery.id}.pdf');
  }

  Future<void> printSupplierHistory(Supplier supplier, List<Delivery> deliveries) async {
    final document = pw.Document();
    document.addPage(pw.MultiPage(
      pageFormat: PdfPageFormat.a4,
      margin: const pw.EdgeInsets.all(32),
      header: (_) => pw.Text('AL-BNC Ventures - Supplier History', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
      build: (_) => [
        ...
        pw.SizedBox(height: 16),
        pw.Table.fromTextArray(headers: const ['Date', 'Product', 'Bags', 'Weight'], data: deliveries.map((delivery) => [_date(delivery.recordedAt), delivery.product.name, '${delivery.numberOfBags}', '${delivery.totalWeight.toStringAsFixed(1)} kg']).toList()),
        pw.SizedBox(height: 16),
