...
class DeliveryDetailScreen extends StatefulWidget {
  const DeliveryDetailScreen({required this.repository, required this.delivery, super.key});
...
class _DeliveryDetailScreenState extends State<DeliveryDetailScreen> {
  ...
  Future<void> _printReceipt() async {
    try {
      await _receiptService.print(_delivery);
    } catch (error) {
  ...
  Future<void> _sendReceipt() async {
    final smsReceiptService = SmsReceiptService(database: widget.repository.database);
    ...
    try {
      await smsReceiptService.send(_delivery);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Delivery Details'), actions: [IconButton(tooltip: 'Print receipt', onPressed: _printReceipt, icon: const Icon(Icons.print_outlined)), IconButton(tooltip: 'Correct weights', onPressed: _editWeights, icon: const Icon(Icons.edit_outlined))]),
      body: ListView(
